...
document.getElementById("btnSXOS").addEventListener("click", async () => {
  logOutput("Requesting access to device...");
  device = await navigator.usb.requestDevice({
    filters: [{
      vendorId: 0x0955
...
document.getElementById("btnHekate").addEventListener("click", async () => {
  logOutput("Requesting access to device...");
  device = await navigator.usb.requestDevice({
    filters: [{
      vendorId: 0x0955
...
$('#btnOtherPayLoads').click(function () {

  $('#divOtherPayLoads').slideToggle();
});
...
var myRadio = $("input[name=payload]");
